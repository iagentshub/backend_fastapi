...
from app.connections.openai_compatible import (
    _stream_with_retry as _do_openai_stream_with_dns_retry,
)
...
def test_openai_stream_retries_transient_dns_failure():
    ...
    with (
        patch("app.connections.openai_compatible.safe_urlopen", side_effect=[dns_error, response]) as urlopen,
        patch("app.connections.openai_compatible.time.sleep") as sleep,
    ):
...
def test_openai_stream_retries_transient_gateway_failure():
    ...
    with (
        patch(
            "app.connections.openai_compatible.safe_urlopen", side_effect=[gateway_error, response]
        ) as urlopen,
        patch("app.connections.openai_compatible.time.sleep") as sleep,
    ):
        reply, _, _ = _do_openai_stream_with_dns_retry(
...
def test_nvidia_connection_url_accepts_base_or_complete_endpoint(configured, expected):
    assert NvidiaProvider._chat_url(configured) == expected
...
async def test_stream_chat_connection_error_yields_error_event():
    ...
    with patch("app.connections.openai_compatible.safe_urlopen", side_effect=Exception("network error")):
        events = [
            e
            async for e in stream_chat(
                agent, conn, [{"role": "user", "content": "Hola"}], _skill_storage()
...
async def test_stream_chat_explains_openai_compatible_404():
    ...
    with patch("app.connections.openai_compatible.safe_urlopen", side_effect=not_found):
        events = [
            event
