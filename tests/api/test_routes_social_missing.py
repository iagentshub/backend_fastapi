...
def test_fork_knowledge_ok(client):
    ...
    r = client.post(f"/api/knowledge/{kid}/fork")
    assert r.status_code == 200
    data = r.json()
...
def test_fork_knowledge_inexistente(client):
    """Líneas 572-574: knowledge no encontrado devuelve 404."""
    _login(client, _uid("fkw2"))
    r = client.post("/api/knowledge/knowledge-no-existe-xyz999/fork")
    assert r.status_code == 404


def test_fork_knowledge_no_publico(client):
    """Líneas 576-583: knowledge existe pero no es público → 403."""
    user = _login(client, _uid("fkw3"))
...
def test_fork_agent_privado_ok(client):
    ...
    _login(client, forker)
    r = client.post(f"/api/agents/private/{agent_id}/fork")
    assert r.status_code == 200
    data = r.json()
    assert data["ok"] is True
...
def test_fork_agent_no_encontrado(client):
    """Líneas 681-683: agente no encontrado → 404."""
    _login(client, _uid("fagnotf"))
    r = client.post("/api/agents/private/agente-no-existe-xyz999/fork")
    assert r.status_code == 404


def test_fork_agent_privado_no_publico(client):
    """Líneas 686-693: agente existe en private pero no en resource_social → 403."""
    _login(client, _uid("fagpriv"))
    r = client.post("/api/agents", json={"name": f"NotPublicFork {uuid4().hex[:6]}"})
    assert r.status_code == 200
    agent_id = r.json()["id"]
    # NO se llama visibility → no está en resource_social

    forker = _uid("fagpriv_fk")
    _login(client, forker)
...
def test_fork_skill_ok(client):
    ...
    forker = _uid("fsk1_fk")
    _login(client, forker)
    r = client.post(f"/api/skills/private/{skill_id}/fork")
    assert r.status_code == 200
    data = r.json()
    assert data["ok"] is True
...
def test_fork_skill_no_encontrada(client):
    """Línea 751: skill no encontrada → 404."""
    _login(client, _uid("fsknf"))
    r = client.post("/api/skills/private/skill-no-existe-xyz999/fork")
    assert r.status_code == 404
...
def test_convert_agent_link_to_fork_sin_enlace(client):
    ...
    _login(client, _uid("conv2"))
    r = client.post("/api/agents", json={"name": f"NoLinkConv {uuid4().hex[:6]}"})
    assert r.status_code == 200
    agent_id = r.json()["id"]
    # Sin fila en resource_social con linked_to_id → 404
    r2 = client.post(f"/api/agents/private/{agent_id}/link/convert-to-fork")
    assert r2.status_code == 404


def test_convert_agent_link_to_fork_ok(client):
    """Líneas 1105-1149: convertir enlace de agente a fork."""
    user = _login(client, _uid("conv1"))
    r = client.post("/api/agents", json={"name": f"ConvAgent {uuid4().hex[:6]}"})
    assert r.status_code == 200
    agent_id = r.json()["id"]
...
def test_convert_skill_link_to_fork_sin_enlace(client):
    ...
    r2 = client.post(f"/api/skills/private/{skill_id}/link/convert-to-fork")
    assert r2.status_code == 404
