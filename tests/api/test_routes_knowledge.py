...
def test_folder_crud_assignment_and_detach(alice):
    ...
    visibility = alice.put(
        ...
        json={"is_public": True, "category": "Research"},
    )
    assert visibility.status_code == 200
    assert visibility.json()["is_public"] is True

    listed_public = alice.get(
        "/api/knowledge/folders", params={"section": "url"}
    ).json()
    assert listed_public[0]["visibility"] == "public"

    preview = alice.get(f"/api/explore/knowledge/{folder['id']}/preview")
    assert preview.status_code == 200
    assert preview.json()["type"] == "folder"
