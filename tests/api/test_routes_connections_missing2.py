...
def _build_hub_mock(conn_data=None, agent_summaries=None, agent_details=None, skill_data=None, know_data=None):
    """Construye AsyncMock de httpx.AsyncClient que enruta respuestas por fragmento de URL."""

    async def _mock_get(url, headers=None):
        resp = MagicMock()
        resp.raise_for_status = MagicMock()
        if "/api/agents?" in url:
            resp.json = MagicMock(return_value=agent_summaries or [])
            ...
            resp.json = MagicMock(return_value=skill_data or [])
        elif "/api/knowledge" in url:
            resp.json = MagicMock(return_value=know_data or [])
        elif "/api/connections" in url:
            resp.json = MagicMock(return_value=conn_data or [])
        else:
...
def test_test_all_no_provider_returns_ok_false(admin_client):
    ...
    with patch("app.api.routes.connections.get_provider", return_value=None):
...
def test_hub_sync_non_admin_user_branch(client):
    ...
    with patch("app.connections.iagentshub._login", side_effect=Exception("auth error")):
...
def test_hub_sync_imports_remote_connections(admin_client):
    ...
    remote_conns = [
        {"id": "rc-m2-1", "name": "Remote OpenAI M2", "type": "openai", "model": "gpt-4o"},
    ]
    mock = _build_hub_mock(conn_data=remote_conns)
    with patch("app.connections.iagentshub._login", return_value="tok"), \
         patch("httpx.AsyncClient", return_value=mock):
...
def test_hub_sync_updates_existing_remote_connection(admin_client):
    ...
    hub_conn = _create_conn(admin_client, _CONN_IAGENTSHUB)
    remote_conns = [{"id": "rc-upd-m2", "name": "Updatable Conn M2", "type": "openai", "model": "gpt-4o"}]

    mock1 = _build_hub_mock(conn_data=remote_conns)
    with patch("app.connections.iagentshub._login", return_value="tok"), \
         patch("httpx.AsyncClient", return_value=mock1):
    ...
    with patch("app.connections.iagentshub._login", return_value="tok"), \
         patch("httpx.AsyncClient", return_value=mock2):
...
def test_hub_sync_safe_name_conflict_adds_hub_label(admin_client):
    ...
    remote_conns = [{"id": "rc-cnf-m2", "name": "Conflicting Name M2", "type": "openai", "model": "gpt-4o"}]
    mock = _build_hub_mock(conn_data=remote_conns)
    with patch("app.connections.iagentshub._login", return_value="tok"), \
         patch("httpx.AsyncClient", return_value=mock):
        r = admin_client.post(f"/api/connections/{hub_conn['id']}/hub-sync")
...
def test_hub_sync_safe_name_double_conflict_adds_number(admin_client):
    ...
    _create_conn(admin_client, {**_CONN_OPENAI, "name": f"DblConflict M2 ({hub_label})", "model": "gpt-3.5"})
    remote_conns = [{"id": "rc-dbl-m2", "name": "DblConflict M2", "type": "openai", "model": "gpt-4o"}]
    mock = _build_hub_mock(conn_data=remote_conns)
    with patch("app.connections.iagentshub._login", return_value="tok"), \
         patch("httpx.AsyncClient", return_value=mock):
        r = admin_client.post(f"/api/connections/{hub_conn['id']}/hub-sync")
...
def test_hub_sync_imports_remote_agents(admin_client):
    ...
    details = [{"id": "ra-m2-1", "name": "Remote Agent M2", "description": "A remote agent"}]
...
def test_hub_sync_updates_existing_agent_via_hub_source(admin_client):
    ...
    local_agents_with_src = [
        {"id": "agent-to-update-m2b", "name": "Agent To Update M2B", "_hub_source": src_key}
    ]

    mock = _build_hub_mock(agent_summaries=summaries, agent_details=details)
    with patch("app.connections.iagentshub._login", return_value="tok"), \
         patch("httpx.AsyncClient", return_value=mock), \
         patch("app.api.routes.connections._agent_storage.list", return_value=local_agents_with_src):
...
def test_hub_sync_imports_remote_skills(admin_client):
    ...
    hub_conn = _create_conn(admin_client, _CONN_IAGENTSHUB)
    skill_data = [{"id": "rs-m2-1", "name": "Remote Skill M2", "description": "A skill"}]
...
def test_hub_sync_imports_remote_knowledge(admin_client):
    ...
    hub_conn = _create_conn(admin_client, _CONN_IAGENTSHUB)
    know_data = [{"id": "rk-m2-1", "title": "Remote KB M2", "type": "url", "content": "http://example.com"}]
...
def test_hub_sync_full_all_types_ok(admin_client):
    ...
    mock = _build_hub_mock(
        conn_data=[{"id": "rc-full-m2", "name": "Full Conn M2", "type": "openai", "model": "gpt-4o"}],
        agent_summaries=summaries,
        agent_details=details,
        skill_data=[{"id": "rs-full-m2", "name": "Full Skill M2", "description": ""}],
        know_data=[{"id": "rk-full-m2", "title": "Full KB M2", "type": "url", "content": ""}],
    )
    with patch("app.connections.iagentshub._login", return_value="tok"), \
         patch("httpx.AsyncClient", return_value=mock):
        r = admin_client.post(f"/api/connections/{hub_conn['id']}/hub-sync")
...
def test_import_models_non_admin_user(client):
    ...
    conn = _create_conn(client, _CONN_OPENAI)
    with patch("app.api.routes.accounts._fetch_models", new_callable=AsyncMock, return_value=["gpt-4o"]):
...
def test_import_models_copies_host_and_url(admin_client):
    ...
    conn = _create_conn(admin_client, {
        "type": "openai",
    ...
    with patch("app.api.routes.accounts._fetch_models", new_callable=AsyncMock, return_value=["gpt-4o-mini"]):
...
def test_resolve_connections_includes_shared_from_group(admin_client):
    ...
    from app.auth.auth import create_token, register_user
    asyncio.run(register_user("shared_u_m2", "pass1234", email="shared_u_m2@example.com"))
...
def test_fetch_ollama_models_oserror_no_alt_host(client):
    ...
    with patch("app.connections.ollama.OllamaProvider._fetch_tags", side_effect=OSError("refused")):
        r = client.get("/api/connections")
    assert r.status_code == 200
...
def test_fetch_ollama_models_generic_exception_returns_empty(client):
    ...
    client.post("/api/connections", json=_CONN_OLLAMA_BASE)
    with patch("app.connections.ollama.OllamaProvider._fetch_tags", side_effect=ValueError("unexpected")):
        r = client.get("/api/connections")
    assert r.status_code == 200
