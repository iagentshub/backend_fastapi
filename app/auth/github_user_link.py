...
async def get_or_create_github_user(
    ...
    async with open_db() as conn:
        row = await conn.fetchone(
            sql("queries/github_user_link:by_provider_sub"),
            ("github", github_id),
        )
        if row:
            return dict(row)
        ...
        suffix = 1
        while await conn.fetchone(sql("queries/github_user_link:username_exists"), (username,)):
            suffix += 1
            username = f"{base_username}{suffix}"[:32]

        account_email = (email or "").strip().lower()
        if not account_email or await conn.fetchone(
            sql("queries/github_user_link:email_exists"), (account_email,)
        ):
        ...
        async with conn.transaction():
            await conn.execute(
                sql("queries/github_user_link:insert_user"),
                (
                    user_id,
                    username,
                    account_email,
