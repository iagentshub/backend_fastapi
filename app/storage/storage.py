...
# ─── ToolStorage ──────────────────────────────────────────────────────────────
# DB-backed. Herramientas ejecutables (scripts python/shell o binarios cpp
# precompilados) que se pueden asignar a un agente. Fase 1: solo catalogación
# y asignación — sin motor de ejecución (ver plan de la Fase 2).
...
class ToolStorage(ResourceStorage):
    ...
    async def save(
        ...
        actual_owner = owner_id or "admin"
        now = _now()
        existing = await self.get_any(tool_id, owner_id=actual_owner)
        if "labels" in payload:
            labels = [str(label) for label in (payload.get("labels") or []) if label]
        elif existing:
            labels = [str(label) for label in (existing.get("labels") or []) if label]
